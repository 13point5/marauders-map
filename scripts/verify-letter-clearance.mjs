...
const variant=process.argv[2]||'quiet';
const pieces=reconstructTower(kit,font,layout,variant);
...
const report={variant,capitals:kit.filter(g=>g.id.length===1).length,cursive:Object.keys(font.glyphs).filter(c=>c!==' ').length,treads:pieces.filter(p=>p.kind==='stair').length,overlappingInkPixels:overlap,treadPixelsWithin3pxOfLetters:withinThreePixels};
await writeFile(`outputs/alphabet/clearance-${variant}.json`,JSON.stringify(report,null,2));console.log(report);
