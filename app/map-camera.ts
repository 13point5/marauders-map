...
export function anchoredScroll({scrollX,scrollY,viewWidth,viewHeight,mapWidth,oldZoom,newZoom,anchorX,anchorY}:{scrollX:number;scrollY:number;viewWidth:number;viewHeight:number;mapWidth:number;oldZoom:number;newZoom:number;anchorX:number;anchorY:number}) {
  ...
  return {x:axis(scrollX,viewWidth,mapWidth,anchorX),y:axis(scrollY,viewHeight,mapWidth*.6,anchorY)};
}
