...
import {WingArt} from './letter-kit/wing-art';
...
export default function TowerStudy(){
 ...
 const openNotes=()=>notes.current?.showModal();
 ...
 useEffect(()=>{const el=viewport.current;if(!el)return;const measure=()=>setMapWidth(Math.min(el.clientWidth/(study==='wing'?.9:study==='reconstruction'?(entranceDetail?.82:.56):study==='reference'?.56:.72),el.clientHeight*5/3));const observer=new ResizeObserver(measure);observer.observe(el);measure();return()=>observer.disconnect()},[study,entranceDetail]);
 ...
 return <main className="tower-study">
  ...
  <div ref={viewport} className="map-viewport study-viewport" role="region" tabIndex={0} aria-label="Map study. Drag to pan, pinch or Control-scroll to zoom. Arrow keys pan; zero fits.">
   <div className="study-paper" style={{width:mapWidth*zoom}}>
    <svg viewBox="0 0 1000 600" className="tower-canvas" role={study==='wing'?'group':'img'} aria-label={study==='wing'?'An Astronomy Wing made of quill lettering with a tower, passage, and Room of Questions':original?'An original floor plan built from reusable letter shapes':study==='reconstruction'?'Reference tower reconstructed from reusable letter pieces':study==='letters'?'The reusable hand-lettered vector kit':'The approved reference tower'}>
     {study==='wing'?<WingArt walking={walking} onOpen={openNotes}/>:study==='reconstruction'?<svg x={entranceDetail?120:240} y="36" width={entranceDetail?760:520} height="528" viewBox={entranceDetail?"550 770 510 340":"60 65 1010 1025"}>
