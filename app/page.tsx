...
type Study='observatory'|'gallery'|'reference'|'letters';
export default function TowerStudy(){
 const [study,setStudy]=useState<Study>('observatory'),[mode,setMode]=useState<Mode>('trace'),[seed,setSeed]=useState(13);
 ...
 useEffect(()=>{const el=viewport.current;if(!el)return;const measure=()=>setMapWidth(Math.min(el.clientWidth/(study==='reference'?.56:.72),el.clientHeight*5/3));const observer=new ResizeObserver(measure);observer.observe(el);measure();return()=>observer.disconnect()},[study]);
 ...
 return <main className="tower-study">
  <header className="study-header"><div><span className="study-folio">II · LETTERS INTO ARCHITECTURE</span><h1>A new piece of the map</h1></div><span className="study-source">New floor plans · ink drawn from your reference</span></header>
  <nav className="study-modes study-choices" aria-label="Choose a design study">{([['observatory','New observatory'],['gallery','New gallery'],['reference','Reference tower'],['letters','Letter kit']] as [Study,string][]).map(([s,label])=><button key={s} aria-pressed={study===s} onClick={()=>changeStudy(s)}>{label}</button>)}</nav>
  <div className="study-secondary">{study==='reference'?<nav className="reference-modes" aria-label="Compare with the photograph">{(['trace','photo','overlay'] as Mode[]).map(m=><button key={m} aria-pressed={mode===m} onClick={()=>setMode(m)}>{m==='trace'?'The drawing':m==='photo'?'Your photograph':'Compare overlay'}</button>)}</nav>:original?<button className="reletter" onClick={()=>setSeed(s=>s+1)}><Shuffle size={14}/> Rearrange the lettering</button>:<p>Each piece can be moved, rotated, and scaled independently.</p>}</div>
  <div ref={viewport} className="map-viewport study-viewport" role="region" tabIndex={0} aria-label="Map study. Drag to pan, pinch or Control-scroll to zoom. Arrow keys pan; zero fits.">
   <div className="study-paper" style={{width:mapWidth*zoom}}>
    <svg viewBox="0 0 1000 600" className="tower-canvas" role="img" aria-label={original?'An original floor plan built from reusable letter shapes':study==='letters'?'The reusable hand-lettered vector kit':'The approved reference tower'}>
     {original?<OriginalBuilding plan={study} seed={seed}/>:study==='letters'?<LetterKit/>:<>
